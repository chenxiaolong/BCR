...
import link.cure.recorder.BuildConfig
import link.cure.recorder.utils.DaysRetention
import link.cure.recorder.utils.NoRetention
import link.cure.recorder.utils.Retention
import link.cure.recorder.utils.Preferences
import link.cure.recorder.utils.format.Encoder
import link.cure.recorder.utils.format.Format
import link.cure.recorder.utils.format.SampleRate
import link.cure.recorder.utils.frameSizeInBytesCompat
...
class RecorderThread(
            ...
            if (isCancelled) {
                ...
                try {
                    ...
                    if (finalFilename != initialFilename) {
                        Log.i(
                            tag,
                            "Renaming ${redact(initialFilename)} to ${redact(finalFilename)}"
                        )
                        ...
                        if (outputFile.renameTo(finalFilename)) {
                            ...
                            Log.w(
                                tag,
                                "Failed to rename to final filename: ${redact(finalFilename)}"
                            )
                        }
                    }
    ...
    companion object {
        ...
        private const val TAG = "RecorderThread"
        ...
        fun timestampFromFilename(name: String): String? {
            try {
                return name.split("_")[0]
            } catch (e: Exception) {
                Sentry.captureException(e)
                Log.e(TAG, "Got an exception while fetching timestamp!", e)
            }
            return null
        }

        fun directionFromFilename(name: String): String? {
            try {
                return when (name.split("_")[1]) {
                    "in" -> "INCOMING"
                    "out" -> "OUTGOING"
                    else -> "UNKNOWN"
                }
                ...
                Sentry.captureException(e)
                Log.e(TAG, "Got an exception while fetching direction!", e)
            }
            return null
        }


        fun phoneFromFilename(name: String): String? {
            try {
                return (name.split("_")[2]).split(".")[0]
            } catch (e: Exception) {
                Sentry.captureException(e)
                Log.e(TAG, "Got an exception while fetching phone!", e)
            }
            return null
        }
