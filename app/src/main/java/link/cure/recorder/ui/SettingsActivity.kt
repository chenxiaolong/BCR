...
import link.cure.recorder.utils.format.Format
import link.cure.recorder.utils.format.NoParamInfo
import link.cure.recorder.utils.format.RangedParamInfo
import link.cure.recorder.utils.format.SampleRate
import link.cure.recorder.utils.LongClickablePreference
...
class SettingsActivity : AppCompatActivity() {
    ...
    private lateinit var callRecordingUploadWorker: CallRecordingUploadTaskQueue
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        callEventUploadTaskQueue =
            CallEventUploadTaskQueue.getInstance(context = applicationContext)
        callRecordingUploadWorker =
            CallRecordingUploadTaskQueue.getInstance(context = applicationContext)
        Log.d(TAG, "Current Queue size:- ${callEventUploadTaskQueue.size()}")

        val eventButton: Button = findViewById(R.id.btn_event_queue)
        eventButton.text = "Current Event Queue Size :- ${callEventUploadTaskQueue.size()}. Clear?"
        eventButton.setOnClickListener {
            Log.d(TAG, callEventUploadTaskQueue.size().toString())
            if (callEventUploadTaskQueue.size() > 0) {
                Log.d(TAG, callEventUploadTaskQueue.peek().toString())
                callEventUploadTaskQueue.remove()
                eventButton.text =
                    "Current Event Queue Size :- ${callEventUploadTaskQueue.size()}. Clear?"
            }
        }
        val recordingButton: Button = findViewById(R.id.btn_recording_queue)
        recordingButton.text = "Current Recording Queue Size :- ${callRecordingUploadWorker.size()}. Clear?"
        recordingButton.setOnClickListener {
            ...
            if (callEventUploadTaskQueue.size() > 0) {
                ...
                recordingButton.text =
                    "Current Recording Queue Size :- ${callRecordingUploadWorker.size()}. Clear?"
            }
        }
    }
